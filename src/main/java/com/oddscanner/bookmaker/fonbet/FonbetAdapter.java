// File: src/main/java/com/oddscanner/bookmaker/fonbet/FonbetAdapter.java
...
public class FonbetAdapter implements BookmakerAdapter {
    ...
    private static final String BASE_URL = "https://www.fon.bet";
    ...
    public List<RawEvent> fetchEvents() {
        ...
        try (Playwright playwright = Playwright.create()) {
            // Настройка браузера (headless, user-agent, viewport)
            Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions()
                    .setHeadless(true) // Всегда используем headless в проде
                    .setArgs(List.of("--no-sandbox", "--disable-dev-shm-usage"))); // Исправлено: List.of()

            BrowserContext context = browser.newContext(new Browser.NewContextOptions()
                    .setViewportSize(1920, 1080) // Установим размер окна
                    .setUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36")); // Поддельный User-Agent
            ...
            // Пример: String htmlContent = page.content(); // Получить весь HTML
            // Затем использовать JSoup или Playwright Locator API для поиска элементов
            // и извлечения данных (названия команд, время начала, ID события и т.д.)
            // ...
            // RawEvent event = new RawEvent(...);
            // events.add(event);
            // ----------------------------------------------------

            browser.close(); // Закрываем браузер после завершения
        } catch (Exception e) {
            log.error("Error fetching events from {}", code(), e);
            // В реальной системе нужен более продвинутый ретраи и обработчик ошибок
            // (например, ошибка капчи, бан IP и т.п.)
        }
        return events;
    }
    ...
    public List<RawMarket> fetchMarkets(String externalEventId) {
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            // Пример: String eventUrl = BASE_URL + "/line/event/" + externalEventId;
            // page.navigate(eventUrl);
            // String htmlContent = page.content();
            // Извлечь рынки (тип, период, линию, исходы) из HTML
